...
export default {
    config: {
        ...
        enableOnFolder: {
            type: 'boolean',
            default: true,
            description: 'Double click on a folder to open it, Double click on a file to open it, otherwise single click to open it.'
        }
    },
    observeConfig() {
            ...
            this.treeView.entryClicked = function(e) {
                ...
                if (e.detail == 1) {
                    ...
                    if (ctx.isDirectory(entry) && config.enableOnFolder) {
                        entry.toggleExpansion(isRecursive);
                    } else if (ctx.isFile(entry) && config.enableOnFile) {
